...
| 拼版（折手） | 排好的页按装订方式摆到大纸：**骑马钉**（套帖对折骑钉，页码外大内小）/ **胶装**（书帖叠放铣背胶装、三面裁边）；每张纸正反各 2 页，标注正反面、书脊折口/裁口与折起后朝外的一面；末尾凑印张自动补空白页（**不占正文页码**）；一键核对漏页/重页/页序/补白位置/朝向；换装订或改大纸自动重排；真实尺寸大纸可打印与导出 SVG/PNG（正反交替、短边翻转） |
...
```bash
npm test           # 单元测试（vitest）：转换/换行/拼版折手/BRF 校验/性能，311 条
...
- 单元测试覆盖：200+ 条转换用例、30 组换行/分页用例、31 条拼版折手用例（骑马钉/胶装版序、补白、漏页/重页/页序/朝向核对、大纸尺寸、大纸 SVG 输出）、BRF 结构校验、1 万字 < 300ms 性能
- e2e 覆盖：全流程（输入→转换→多音字确认→导出解锁→BRF 下载）、键盘流、刷新持久化、逐方编辑、违规报告、100+ 页长文滚动、模板、词语表、设置持久化、拼版折手（两种装订切换/补白提示/正反面与朝外向标注/8 页经典版序）
...
│   │   ├── imposition.ts           #   拼版折手（骑马钉套帖 / 胶装叠帖、末尾补白、漏页重页页序核对）
│   │   ├── imposition-svg.ts       #   拼版大纸真实尺寸 SVG（双联正反、折线/裁口辅助标注）
...
│   ├── pages/                      # 首页 / 编辑器 / 打印（单页 + 拼版大纸）/ 模板与词语表 / 设置
│   └── components/                 # 点阵方（SVG）、分页预览、多音字面板、拼版折手面板
├── tests/                          # 单元测试（vitest）
