...
| 拼版折手 | 把正文页按装订方式摆到大纸：骑马钉（外套帖）/胶装（叠帖），一张纸正反各两页对折成 4 页一帖；末尾补空白页（不占正文页码）；标注正反面/折缝/页码角标/帖标；换装订或纸张即时重排；自动核对漏页、重页、折后页序 |
...
```bash
npm test           # 单元测试（vitest）：转换/换行/拼版折手/BRF 校验/性能
npm run test:watch # 监听模式
npm run e2e        # Playwright e2e：含拼版折手用例，自动 build + preview（独占端口 4317）
E2E_BASE_URL=http://localhost:8097 npm run e2e   # e2e 直接打容器，验证生产镜像
```

- 单元测试覆盖：200+ 条转换用例、30 组换行/分页用例、拼版折手（骑马钉/胶装摆版、补白、折后阅读顺序、漏页/重页/页序/结构核对）、BRF 结构校验、1 万字 < 300ms 性能
- e2e 覆盖：全流程（输入→转换→多音字确认→导出解锁→BRF 下载）、键盘流、刷新持久化、逐方编辑、违规报告、100+ 页长文滚动、模板、词语表、设置持久化、拼版折手（装订/纸张切换重排与核对）
...
│   │   ├── imposition.ts           #   拼版折手（骑马钉/胶装摆版、补白、折后顺序、核对）
│   │   ├── imposition-svg.ts       #   拼版大纸真实尺寸 SVG（折缝/页码角标/帖标）
...
- 拼版以「一张大纸正反各 2 页、竖缝对折成 4 页一帖」为模型；骑马钉外套帖（每向内一层版位向书心收 2）、胶装顺序成帖；总版数凑 4 的倍数，末尾补白不占正文页码。核对独立于生成过程，按「折好后逐位置必须是第 i 页或末尾补白」严格校验，能抓出漏页/重页/页序错/补白错位/越界页码/张数不符
